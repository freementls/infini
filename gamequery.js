...
(function($) {
    ...
    $.extend({ gameQuery: {
        ...
        Animation: function (options, imediateCallback) {
            ...
            this.numberOfFrames = options.numberOfFrames; // The number of frames to be displayed when playing the animation
                    ...
                    if(gameQuery.animation){
                        ...
                        if ( (gameQuery.idleCounter == gameQuery.animation.rate-1) && gameQuery.playing){
                            ...
                            if(gameQuery.animation.type & $.gameQuery.ANIMATION_ONCE){
                                if(gameQuery.currentFrame < gameQuery.animation.numberOfFrames-1){
                                    gameQuery.currentFrame += gameQuery.frameIncrement;
                                } else if(gameQuery.currentFrame == gameQuery.animation.numberOfFrames-1) {
                                ...
                                if(gameQuery.animation.type & $.gameQuery.ANIMATION_PINGPONG){
                                    if(gameQuery.currentFrame == gameQuery.animation.numberOfFrames-1 && gameQuery.frameIncrement == 1) {
                                ...
                                gameQuery.currentFrame = (gameQuery.currentFrame+gameQuery.frameIncrement)%gameQuery.animation.numberOfFrames;
                            ...
                            if((gameQuery.animation.type & $.gameQuery.ANIMATION_VERTICAL) && (gameQuery.animation.numberOfFrames > 1)){
                                if(gameQuery.multi){
                            ...
                            } else if((gameQuery.animation.type & $.gameQuery.ANIMATION_HORIZONTAL) && (gameQuery.animation.numberOfFrames > 1)) {
                                if(gameQuery.multi){
                ...
                if(this.gameQuery != undefined){
                    ...
                    if($.isArray(gameQuery.frameTracker)){
                        for(var i=0; i<gameQuery.frameTracker.length; i++){
                            ...
                            if(gameQuery.idleCounter[i] == gameQuery.animations[i].rate-1){
                                ...
                                if(gameQuery.animations[i].type & $.gameQuery.ANIMATION_ONCE){
                                    if(gameQuery.frameTracker[i] < gameQuery.animations[i].numberOfFrames-1){
                                        gameQuery.frameTracker[i] += gameQuery.frameIncrement[i];
                                    }
                                } else {
                                    if(gameQuery.animations[i].type & $.gameQuery.ANIMATION_PINGPONG){
                                        if(gameQuery.frameTracker[i] == gameQuery.animations[i].numberOfFrames-1 && gameQuery.frameIncrement[i] == 1) {
                                            gameQuery.frameIncrement[i] = -1;
                                        } else if (gameQuery.frameTracker[i] == 0 && gameQuery.frameIncrement[i] == -1) {
                                            gameQuery.frameIncrement[i] = 1;
                                        }
                                    }
                                    gameQuery.frameTracker[i] = (gameQuery.frameTracker[i]+gameQuery.frameIncrement[i])%gameQuery.animations[i].numberOfFrames;
                                }
                            }
                        ...
                        if(gameQuery.idleCounter == gameQuery.animations.rate-1){
                            ...
                            if(gameQuery.animations.type & $.gameQuery.ANIMATION_ONCE){
                                if(gameQuery.frameTracker < gameQuery.animations.numberOfFrames-1){
                                    gameQuery.frameTracker += gameQuery.frameIncrement;
                                }
                            } else {
                                if(gameQuery.animations.type & $.gameQuery.ANIMATION_PINGPONG){
                                    if(gameQuery.frameTracker == gameQuery.animations.numberOfFrames-1 && gameQuery.frameIncrement == 1) {
                                        gameQuery.frameIncrement = -1;
                                    } else if (gameQuery.frameTracker == 0 && gameQuery.frameIncrement == -1) {
                                        gameQuery.frameIncrement = 1;
                                    }
                                }
                                gameQuery.frameTracker = (gameQuery.frameTracker+gameQuery.frameIncrement)%gameQuery.animations.numberOfFrames;
                            }
                        }
                    ...
                    $(this).find("."+$.gameQuery.tileCssClass).each(function(){
                        if($.isArray(gameQuery.frameTracker)){
                            ...
                            if((gameQuery.animations[animationNumber].type & $.gameQuery.ANIMATION_VERTICAL) && (gameQuery.animations[animationNumber].numberOfFrames > 1)){
                            ...
                            } else if((gameQuery.animations[animationNumber].type & $.gameQuery.ANIMATION_HORIZONTAL) && (gameQuery.animations[animationNumber].numberOfFrames > 1)) {
                            ...
                            if((gameQuery.animations.type & $.gameQuery.ANIMATION_VERTICAL) && (gameQuery.animations.numberOfFrames > 1)){
                            ...
                            } else if((gameQuery.animations.type & $.gameQuery.ANIMATION_HORIZONTAL)  && (gameQuery.animations.numberOfFrames > 1)) {
    ...
    $.fn.extend({
        ...
        playground: function(options) {
            if(this.length == 1){
				...
				// Add the touchTracker to the gameQuery object:
                $.gameQuery.touchTracker = {
                    x: 0,
                    y: 0};
                // We only enable the real tracking if the users wants it
                //var scenegraphOffset = $.gameQuery.playground.offset();
                if(options.touchTracker){
                    $($.gameQuery.playground).touchmove(function(event){
                        //alert('touchmove');
						$.gameQuery.touchTracker.x = event.changedTouches[0].pageX - scenegraphOffset.left; // reference first touch point (ie: first finger)
                        $.gameQuery.touchTracker.y = event.changedTouches[0].pageY - scenegraphOffset.top; // reference first touch point (ie: first finger)
                    });
					
					/*box1.addEventListener('touchmove', function(e){
		var touchobj = e.changedTouches[0] // reference first touch point (ie: first finger)
		var xdist = parseInt(touchobj.clientX) - startx
		var ydist = parseInt(touchobj.clientY) - starty
		statusdiv.innerHTML = 'Status: touchmove<br /> Horizontal distance traveled: ' + xdist + 'px, Vertical distance traveled: ' + ydist + 'px'
		e.preventDefault()
	}, false)*/
					
                    $(document).touchstart(function(event){
						//alert('touchstart');
						//$.gameQuery.touchTracker[event.changedTouches[0].which] = true;
						$.gameQuery.touchTracker.x = event.changedTouches[0].pageX - scenegraphOffset.left; // reference first touch point (ie: first finger)
                        $.gameQuery.touchTracker.y = event.changedTouches[0].pageY - scenegraphOffset.top; // reference first touch point (ie: first finger)
                    });
                    $(document).touchend(function(event){
                        //alert('touchend');
						//$.gameQuery.touchTracker[event.changedTouches[0].which] = false;
						$.gameQuery.touchTracker.x = event.changedTouches[0].clientX - scenegraphOffset.left; // reference first touch point (ie: first finger)
                        $.gameQuery.touchTracker.y = event.changedTouches[0].clientY - scenegraphOffset.top; // reference first touch point (ie: first finger)
						// e.preventDefault()
				...
				/*
				// Add the pointerTracker to the gameQuery object:
                $.gameQuery.pointerTracker = {
                    x: 0,
                    y: 0};
                // We only enable the real tracking if the users wants it
                var scenegraphOffset = $.gameQuery.playground.offset();
                if(options.pointerTracker){
                    $($.gameQuery.playground).pointermove(function(event){
                        //alert('pointermove');
						$.gameQuery.pointerTracker.x = event.pageX - scenegraphOffset.left;
                        $.gameQuery.pointerTracker.y = event.pageY - scenegraphOffset.top;
                    });
                    $(document).pointerdown(function(event){
						//alert('pointerdown');
						$.gameQuery.pointerTracker[event.which] = true;
                    });
                    $(document).pointerup(function(event){
                        //alert('pointerup');
						$.gameQuery.pointerTracker[event.which] = false;
                    });
                }*/
