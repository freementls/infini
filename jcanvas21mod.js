...
// Draws horseshow
$.fn.drawHorseshoe = function drawHorseshoe(args) {
	var $canvases = this, e, ctx,
		params,
		angle, dx, dy,
		temp_var;
	...
	for (e = 0; e < $canvases.length; e += 1) {
		...
		if (ctx) {
			...
			params = new jCanvasObject(args);
			_addLayer($canvases[e], params, args, drawHorseshoe);
			if (params.visible) {
				if (params.radius2 < params.radius1) {
					temp_var = params.radius2;
					params.radius2 = params.radius1;
					params.radius1 = temp_var;
				}

				_transformShape($canvases[e], ctx, params, params.radius2 * 2);
				_setGlobalProps($canvases[e], ctx, params);
				...
				// Calculate angular position of horseshoe
				angle = ((params.start + params.end) / 2);

				// Calculate ratios for horseshoe's angle
				dx = (params.radius2 * params.spread * cos(angle));
				dy = (params.radius2 * params.spread * sin(angle));

				// Adjust position of horseshoe
				params.x += dx;
				params.y += dy;

				//calculated_x = center_x + (Math.cos(toRadians(jcanvas_degrees_to_math_degrees(degree))) * radius);
				// Calculate intermediate point
				params.intx = params.x + (params.radius1 * cos(params.end));
				params.inty = params.y + (params.radius1 * sin(params.end));

				//alert('params.name, params.x, params.y, params.radius1, params.radius2, params.start, params.end, cos(params.start), sin(params.start), params.ccw, params.intx, params.inty: ' + params.name + ', ' + params.x + ', ' + params.y + ', ' + params.radius1 + ', ' + params.radius2 + ', ' + params.start + ', ' + params.end + ', ' + cos(params.start) + ', ' + sin(params.start) + ', ' + params.ccw + ', ' + params.intx + ', ' + params.inty);
				// Draw horseshoe
				ctx.beginPath();
				ctx.arc(params.x, params.y, params.radius2, params.start, params.end, params.ccw);
				ctx.lineTo(params.intx, params.inty);
				// if (params.start < params.end) {
				// 	params.start += (2 * PI);
				// }
				//alert('params.x, params.y, params.radius1, params.start, params.end, params.ccw: ' + params.x + ', ' + params.y + ', ' + params.radius1 + ', ' + params.start + ', ' + params.end + ', ' + params.ccw);
				//params.end -= (2 * PI);
				ctx.arc(params.x, params.y, params.radius1, params.end, params.start, !params.ccw);
				// Check for jCanvas events
				_detectEvents($canvases[e], ctx, params);
				// Always close path
				params.closed = true;
